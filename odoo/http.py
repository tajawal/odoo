    ...
    def dispatch(self):
        try:
            ...
            if rpc_request_flag or rpc_response_flag:
                ...
                if psutil:
                    end_memory = memory_info(psutil.Process(os.getpid()))
                logline = '%s: %s %s => %s: time:%.3fs mem: %sk -> %sk (diff: %sk)' % (
                    endpoint, model, method, self.params, end_time - start_time, start_memory / 1024, end_memory / 1024, (end_memory - start_memory)/1024)
                if rpc_response_flag:
