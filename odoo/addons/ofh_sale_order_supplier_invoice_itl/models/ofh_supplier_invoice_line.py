...
class OfhSupplierInvoiceLine(models.Model):
    ...
    @api.multi
    def _match_itl_with_sale_order(self):
        self.ensure_one()

        if self.invoice_type != 'itl':
            return

        order_ids = self.env['ofh.sale.order'].search(
            self._get_itl_sale_order_domain())
        ...
        for a_order in amendment_orders:
            self._match_gds_with_sale_order_line(a_order)
            if self.order_id:
                return
        return

    @api.multi
    def _match_itl_with_sale_order_line(self, order_id):
        # Refund an Amendments never matches with Initial Booking.
        self.ensure_one()
        if self.invoice_status == 'RFND':
            self._match_itl_with_payment_request(order_id)
            return
        ...
        for line in order_id.line_ids:
            ...
            if order_id.booking_category == BOOKING_CAT_AMND:
                total_supplier_cost = order_id.total_supplier_cost

                supplier_cost = sum([
                    l.cost_amount for l in order_id.invoice_line_ids])
    ...
    @api.multi
    def _match_itl_with_payment_request(self, order_id):
        self.ensure_one()
    ...
    @api.multi
    def _get_itl_sale_order_domain(self):
        self.ensure_one()
