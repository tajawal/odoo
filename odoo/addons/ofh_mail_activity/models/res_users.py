...
class ResUsers(models.Model):
    ...
    def activity_user_count(self):
        query = """
            ...
            FROM mail_activity AS act
            JOIN ir_model AS m ON act.res_model_id = m.id
            JOIN mail_activity_res_users_rel as mu ON
            mu.mail_activity_id = act.id
            WHERE mu.res_users_id = %(user_id)s
            GROUP BY m.id, states, act.res_model;
        """
        ...
        model_names = {
            n[0]: n[1] for
            n in self.env['ir.model'].browse(model_ids).name_get()}
        ...
        for activity in activity_data:
            ...
            user_activities[
                activity['model']]['%s_count' % activity['states']] += \
                activity['count']
