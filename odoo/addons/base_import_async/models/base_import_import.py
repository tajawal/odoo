...
from odoo.models import TransientModel, fix_import_export_id_paths
...
class BaseImportImport(TransientModel):
    ...
    def _read_csv_attachment(self, att_id, options):
        ...
        f = StringIO(base64.decodestring(att.datas).decode())
        reader = csv.reader(
            f, delimiter=str(options.get(OPT_SEPARATOR)),
            quotechar=str(options.get(OPT_QUOTING)))
