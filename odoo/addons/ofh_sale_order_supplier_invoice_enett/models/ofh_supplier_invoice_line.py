...
class OfhSupplierInvoiceLine(models.Model):
    ...
    def _match_enett_with_sale_order(self):
        ...
        self._match_enett_with_sale_order_line(initial_orders)
        if self.order_id:
            return
        ...
        for a_order in amendment_orders:
            self._match_enett_with_sale_order_line(a_order)
            if self.order_id:
                return
        return

    @api.multi
    def _match_enett_with_sale_order_line(self, order_id):
        # Refund an Amendments never matches with Initial Booking.
        self.ensure_one()
        if self.invoice_status == 'RFND':
            self._match_enett_with_payment_request(order_id)
            return
    ...
    @api.multi
    def _match_enett_with_payment_request(self, order_id):
        self.ensure_one()
