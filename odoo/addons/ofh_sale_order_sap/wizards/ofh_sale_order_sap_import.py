...
class OfhSaleOrderSapImport(models.TransientModel):

    _name = 'ofh.sale.order.sap.import'
    ...
    def import_report(self):
        ...
        if self.report_type == 'va05':
            import_type = self.env.ref(
                'ofh_sale_order_sap.sap_sale_import_type')
            backend = self.env.ref(
                'ofh_sale_order_sap.sap_sale_import_backend')
        else:
            import_type = self.env.ref(
                'ofh_sale_order_sap.sap_payment_import_type')
            backend = self.env.ref(
